...
import com.ray.service.ISeckillVoucherService;
import com.ray.service.IVoucherOrderService;
import com.ray.utils.RedisIdWorker;
...
public class VoucherOrderServiceImpl extends ServiceImpl<VoucherOrderMapper, VoucherOrder> implements IVoucherOrderService {
    ...
    private void initOrderStreamGroup() {
        try {
            ...
            log.info("秒杀订单消费组初始化完成，stream={}, group={}", ORDER_STREAM, ORDER_GROUP);
        } catch (Exception e) {
            if (containsMessage(e, "BUSYGROUP")) {
                log.debug("秒杀订单消费组已存在，stream={}, group={}", ORDER_STREAM, ORDER_GROUP);
                return;
            }
            log.debug("秒杀订单流尚未创建，收到首笔秒杀订单后初始化消费组，stream={}, group={}", ORDER_STREAM, ORDER_GROUP);
        }
    }

    private boolean isMissingOrderGroup(Throwable throwable) {
        return containsMessage(throwable, "NOGROUP") || containsMessage(throwable, "NO such key");
    }
