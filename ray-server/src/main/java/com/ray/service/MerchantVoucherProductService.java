...
public interface MerchantVoucherProductService {
    ...
    /** 创建券型不可变的空草稿。 */
    MerchantVoucherProductVO create(MerchantVoucherProductCreateRequest request);

    /** 查询当前门店商品完整草稿或只读事实。 */
    MerchantVoucherProductVO get(Long productId);

    /** 按乐观锁版本保存完整草稿快照。 */
    MerchantVoucherProductVO update(Long productId, MerchantVoucherProductUpdateRequest request);
