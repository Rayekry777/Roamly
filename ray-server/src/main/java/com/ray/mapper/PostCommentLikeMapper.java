...
public interface PostCommentLikeMapper extends BaseMapper<PostCommentLike> {
    /** 幂等新增评论点赞事实。 */
    @Insert("INSERT IGNORE INTO post_comment_like (comment_id, user_id) VALUES (#{commentId}, #{userId})")
    int insertIgnore(@Param("commentId") Long commentId, @Param("userId") Long userId);

    /** 删除当前用户对评论的点赞事实。 */
    @Delete("DELETE FROM post_comment_like WHERE comment_id = #{commentId} AND user_id = #{userId}")
