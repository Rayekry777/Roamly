...
- 当前源码共 98 个唯一 `operationId`：包含阶段 21 的券审核与商户下架操作。后续操作数以阶段实际 OpenAPI 为准，不在设计阶段伪造完成数。
...
- 阶段 20 已完成 25 张业务表范围内的四类券建券、媒体绑定、复制、提交和商户端体验；阶段 21 增加券审核、上下架和公开销售状态计算，运行时目标为 98 个唯一 `operationId`。
