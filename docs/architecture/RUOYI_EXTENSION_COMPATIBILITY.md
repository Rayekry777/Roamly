...
阶段 16 至 30 所采用的若依兼容能力已在 Demo 范围完成；`extensionImplementationStatus`（扩展实现状态）仅表示生产级可选适配器尚未启用，不影响当前阶段验收。Demo 对 Lock4j、SnailJob 和 Fesod 分别使用 Redisson 原生锁、Spring Scheduling 和自有 OOXML writer，三者均保持可替换的端口边界。
...
| Lock4j | 下单、核销、退款和结算并发协调 | Demo 使用 Redisson 原生 `RLock`；Lock4j 2.2.7 作为生产适配器候选，数据库仍负责最终正确性 | 22、24、26、29 |
| SnailJob | 关单、过期、上下架、清理、退款重试和 T+1 结算 | Demo 使用 Spring Scheduling 与幂等任务服务；SnailJob 1.10.0 作为生产执行器适配器候选 | 23-29 |
| OSS/S3 | 用户媒体、经营资质、券图片和导出文件 | 建立自有存储端口，dev/test 本地，prod 使用 S3 兼容实现 | 18 |
| sms4j | 消费者与商户验证码 | 保留 Mock/禁用模式，生产适配时不改变认证接口 | 生产扩展 |
| Apache Fesod | 管理列表同步 XLSX 导出 | Demo 使用自有 OOXML writer；Fesod 作为生产导出适配器候选，继续使用受权限控制的同步导出且不新增业务任务表 | 29 |
...
- 内置 Spring Scheduling 与未来 SnailJob 均允许重复投递；任务以业务资源 ID 和动作类型构造幂等键，并保留查询/支付惰性关单。
