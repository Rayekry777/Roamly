...
- 小程序页面、组件、状态和交互细节以同级项目 `Roamly-miniapp/MINIAPP_DEVELOPMENT.md` 为准。
...
- 当前数据库有 10 张业务表，结构来源为 `ray-server/src/main/resources/schema-init.sql`。
- 当前未使用 Flyway/Liquibase；本次仍采用有序、不可修改的手工增量 SQL。
- `schema-init.sql` 含 `DROP TABLE`，只允许用于全新开发数据库，禁止在现有数据库或生产数据库执行。
- 新结构真正落库前，[DATABASE_SCHEMA.md](./DATABASE_SCHEMA.md) 继续只记录当前实际结构，不提前写入目标表。
...
以下结构均为“未实现”设计，不是当前数据库事实。实际字段类型、DDL 顺序和数据回填在对应阶段开始前冻结。
...
| 表 | 用途 | 隔离范围 | 状态 |
|---|---|---|---|
| `tb_city` | 可用城市字典 | 平台级 | 未实现 |
| `tb_content_section` | 官方内容分区 | 平台级 | 未实现 |
| `tb_section_follow` | 用户关注分区 | 用户级 | 未实现 |
| `tb_media_asset` | 临时和已绑定媒体 | 用户级 | 未实现 |
| `tb_post` | 统一社区动态 | 城市/用户级 | 未实现 |
| `tb_post_media` | 动态媒体及顺序 | 动态级 | 未实现 |
| `tb_post_like` | 动态点赞事实 | 用户/动态级 | 未实现 |
...
- 不引入 Flyway。
- 增量 SQL 使用日期和顺序号命名，执行后不得修改；修复必须新增下一条脚本。
- 每次执行前备份数据库，在隔离环境验证，再记录环境、脚本、校验值、执行人、时间和结果。
- DDL 与大批量回填拆分，避免长时间锁表。
- `schema-init.sql` 仅维护全新环境最终快照，不作为存量数据库升级脚本。
...
- 将逗号分隔图片拆为媒体资产和 `tb_post_media` 顺序记录；无法读取的文件单独记录，不阻断其余数据迁移。
...
| 1 | 冻结城市、分区、媒体字段与 SQL | OpenAPI、DDL、种子和回退评审完成 | 未实现 |
| 2 | 实现城市、分区、分区关注、临时媒体 | 后端测试、OpenAPI、文档通过 | 未实现 |
| 3 | 冻结统一动态、点赞和 Blog 迁移 | 字段、索引、迁移校验脚本定稿 | 未实现 |
| 4 | 实现 Post、媒体绑定、点赞和数据迁移 | 新接口可用，旧数据核对一致 | 未实现 |
...
- 手工 SQL 缺少迁移框架自动防重能力，必须通过编号、校验值、执行记录和人工评审控制。
