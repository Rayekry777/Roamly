...
- [阶段 14：Demo 直接重构与旧能力退役](./stages/STAGE_14_LEGACY_RETIREMENT.md)
...
| 旧 Blog、Blog 点赞和旧图片上传 | `/v1/blogs/**`、`/v1/blog-images` | 已废弃 | Demo 直接重构后不再保留兼容入口、表或代码 |
| 统一动态、媒体绑定与点赞 | `/v1/posts/**`、`/v1/users/**/posts` | 已实现 | 作为唯一社区内容模型；开发库通过快照重建 |
| Post 推荐、关注与分区信息流 | `/v1/feeds/**`、`/v1/sections/{sectionId}/posts` | 已实现 | 数据库快照和真实数据联调待验收 |
| Post 评论与回复 | `/v1/posts/**/comments`、`/v1/comments/**` | 已实现 | 作为唯一评论模型；运行数据库重建与真机联调待验收 |
| 旧优惠券、秒杀订单 | `/v1/vouchers`、`/v1/seckill-vouchers/**` | 已废弃 | Demo 直接重构后不再保留兼容入口、表或代码 |
| 团购商品、订单与券包 | `/v1/voucher-products/**`、`/v1/users/me/orders`、`/v1/users/me/vouchers` | 已实现 | 作为唯一交易模型 |

### 2.3 当前数据库与迁移风险

- 当前数据库快照有 19 张业务表，结构来源为 `ray-server/src/main/resources/schema-init.sql`；旧 Blog、旧优惠券与旧秒杀表已直接移除，`voucher_order` 仅保留团购订单快照字段。
...
- 已消费标识优先由服务端查询当前用户、当前商户的 `user_voucher.status=USED` 生成；旧 `voucher_order` 已核销记录仅在迁移期作为兜底。
...
- 支付成功内部事件调用不暴露 HTTP 的 `VoucherSettlementService.confirmPaid(orderId, paidTime)`，按订单 ID 幂等发券；调用方必须先验证支付渠道签名、订单归属和金额。券包读取前由服务端将过期未使用券刷新为 `EXPIRED`，不在第一阶段暴露模拟支付 HTTP 接口。
...
- 当前处于 Demo 阶段，明确采用直接覆盖重构：不迁移 Blog、旧优惠券、旧秒杀订单、旧点赞或旧图片数据，也不维护兼容 API、旧表和双写。
- `seed-dev.sql` 直接写入当前城市、分区、用户、商户、Post、PostComment 与团购商品样例；其插入顺序满足逻辑关联，且不依赖旧表转换。
- `schema-init.sql` 仅用于允许清空的开发库。执行后应使用 `seed-dev.sql` 重新生成演示数据；非 Demo 数据库不得直接运行该快照。
...
| 3 | 冻结统一动态、点赞和开发数据 | 字段、索引及直接 Post 种子数据定稿 | 已实现 |
| 4 | 实现 Post、媒体绑定与点赞 | 新接口可用，重建后的开发样例可用 | 开发中 |
...
| 10 | 冻结并实现团购商品、订单和券包 | 库存、限购、订单、发券幂等通过 | 开发中 |
| 11 | 直接退役旧接口和旧表 | 删除旧链路、直接生成当前模型测试数据 | 已实现 |
...
- 开发数据库重建后，Post、评论、商户、团购商品与接口聚合关系一致。
...
| 2026-09-02 | 阶段 3 动态数据契约 | 冻结统一动态、媒体关系与点赞事实；后续已改为直接生成当前模型的开发样例，不再转换旧 Blog |
...
| 2026-09-03 | 阶段 14 Demo 直接重构 | 移除旧 Blog、旧图片上传、旧优惠券与秒杀代码、路由、OpenAPI、表和开发转换 SQL；开发种子直接生成 Post、评论和团购商品，旧数据不迁移 |
...
- 本项目仍处于 Demo 阶段，旧 Blog、旧优惠券、旧秒杀与旧上传 API 已直接退役；开发数据库必须通过当前快照重新生成数据。
