...
本阶段交付平台商户申请审核、审核资料私有读取、门店激活、停用和恢复，形成“游客提交申请 -> 管理员审核 -> 门店营业 -> 账号成为租户并获得经营权限 -> 平台治理”的事务闭环。
...
- 审核通过把申请账号设置为 `TENANT`（租户）、绑定新门店并迁移为 `ACTIVE`（已激活）。
...
- 返回 `AdminShopDetailVO`，包含门店、来源申请、租户、类目、城市、地址、营业时间、账号状态汇总及最近治理信息。
...
5. 条件更新申请账号为 `TENANT`（租户）、绑定门店、迁移为 `ACTIVE`（已激活）并清空停用信息。
...
- 筛选状态、城市、类目和关键词；表格展示门店、租户、类目、城市、账号摘要、经营状态、最近变更和停用原因。
- 详情抽屉展示来源申请、地址、七日营业时间、租户、账号状态计数和治理记录。
...
- 通过事务创建唯一门店、复制快照、把游客激活为租户、递增版本并写审计。
