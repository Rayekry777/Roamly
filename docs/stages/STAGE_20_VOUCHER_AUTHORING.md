...
- 阶段 19 已实现，商户申请审核能创建 `ACTIVE`（营业中）门店并激活 `TENANT`（租户）账号。
...
- 同门店租户或店长可读取商品已绑定媒体；新上传临时媒体只能由上传账号首次绑定，跨门店、跨商品或用途错误均拒绝。
...
- `TENANT`（租户）与 `MANAGER`（店长）且账号和门店均为活动状态时可用全部 7 个接口。
...
- 租户/店长允许、核销员拒绝、三类 Token 隔离、跨店 404、账号和门店停用拒绝。
