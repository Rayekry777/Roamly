...
- 支付成功通过不暴露 HTTP 的 `VoucherSettlementService.confirmPaid(orderId, paidTime)` 接入：调用方必须先完成支付渠道签名与金额核验。该方法只接受待支付订单，条件更新为已支付后按 `uk_user_voucher_order` 幂等写入用户券并累计销量；重复支付事件不会重复发券。
- 券有效期由商品固定区间或支付时间加有效天数计算；随机券码使用安全随机源且不写入日志。
- 券包列表与详情读取前把当前用户的 `UNUSED` 过期券原子刷新为 `EXPIRED`；状态始终由服务端返回，客户端不自行改写。
- 商户点评的消费认证优先查询新用户券事实表中当前商户的 `USED` 券；旧优惠券订单核销记录仅在迁移期兜底。
