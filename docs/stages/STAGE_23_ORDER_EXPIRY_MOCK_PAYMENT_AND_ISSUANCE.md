...
- 服务端在读取门店数据前强制校验 `merchant:order:read`（商户订单查看）权限；`TENANT`（租户）和 `MANAGER`（店长）允许，`VERIFIER`（核销员）明确返回 403 `MERCHANT_FORBIDDEN`（商户权限不足）。
...
- 商户订单页消费 `GET /v1/merchant/orders`，仅对拥有 `merchant:order:read`（商户订单查看）权限的激活租户/店长开放；核销员访问显示权限错误。
