...
- 管理端采用平台管理员、商户审核员、财务管理员三种固定角色；商户端采用游客、租户、店长、核销员四种固定角色。
...
| 端 | 推荐账号 | 开发凭据 |
|---|---|---|
| 消费者小程序 | `13456789011` | Mock 验证码 `123456`；密码 `Roamly123` |
...
1. 复制 `.env.example` 为 `.env`，填写数据库、Redis 和统一本地媒体根目录配置（`MEDIA_STORAGE_ROOT`）；真实 `.env` 已被 Git 忽略。
